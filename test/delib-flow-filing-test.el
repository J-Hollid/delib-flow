...
(defmacro delib-flow-filing-test--with-temp-org-file (content &rest body)
...
(defmacro delib-flow-filing-test--with-temp-project-file (content &rest body)
...
(defmacro delib-flow-filing-test--with-temp-zk-root (files &rest body)
...
(defun delib-flow-filing-test--accept-inspect (run)
...
(defun delib-flow-filing-test--accept-match (run)
...
(defun delib-flow-filing-test--set-filing-selection (run selection &optional notes)
...
(defun delib-flow-filing-test--draft-selected-filing-item (run)
...
(defun delib-flow-filing-test--approve-selected-filing-item (run)
  "Return RUN after drafting and approving its selected filing item."
  (delib-flow--run-stage-locally
   (delib-flow-filing-test--draft-selected-filing-item run)
   'select-approved-filing-actions))

(defun delib-flow-filing-test--set-filing-conflict-resolution (run resolution
...
(defun delib-flow-filing-test--file-buffer-string (file)
  "Return current buffer text for FILE."
  (with-current-buffer (find-file-noselect file)
    (buffer-string)))

(defun delib-flow-filing-test--reference-note-part-editor-run ()
...
(defun delib-flow-filing-test--reference-note-candidate-only-run ()
...
;; Preserve the existing helper names inside moved tests so the aggregate file
;; can keep its remaining local helpers unchanged during migration.
(defmacro delib-flow-test--with-temp-project-file (content &rest body)
  "Compatibility wrapper for filing test project fixtures."
  (declare (indent 1))
  `(delib-flow-filing-test--with-temp-project-file ,content ,@body))

(defmacro delib-flow-test--with-temp-zk-root (files &rest body)
  "Compatibility wrapper for filing test ZK fixtures."
  (declare (indent 1))
  `(delib-flow-filing-test--with-temp-zk-root ,files ,@body))

(defalias 'delib-flow-test--accept-inspect
  #'delib-flow-filing-test--accept-inspect)

(defalias 'delib-flow-test--accept-match
  #'delib-flow-filing-test--accept-match)

(defalias 'delib-flow-test--set-filing-selection
  #'delib-flow-filing-test--set-filing-selection)

(defalias 'delib-flow-test--draft-selected-filing-item
  #'delib-flow-filing-test--draft-selected-filing-item)

(defalias 'delib-flow-test--approve-selected-filing-item
  #'delib-flow-filing-test--approve-selected-filing-item)

(defalias 'delib-flow-test--set-filing-conflict-resolution
  #'delib-flow-filing-test--set-filing-conflict-resolution)

(defalias 'delib-flow-test--file-buffer-string
  #'delib-flow-filing-test--file-buffer-string)

(defalias 'delib-flow-test--reference-note-part-editor-run
  #'delib-flow-filing-test--reference-note-part-editor-run)

(defalias 'delib-flow-test--reference-note-candidate-only-run
  #'delib-flow-filing-test--reference-note-candidate-only-run)
